...
const db = {
  ...
  get: async (query, params = []) => (await client.query(query, params)).rows[0] || null,
  all: async (query, params = []) => (await client.query(query, params)).rows,
};
...
bot.on('message', async (msg) => {
  ...
  if (text === 'Zarządzaj subskrypcjami') { sess.mode = 'manage_subscriptions'; return bot.sendMessage(chatId, 'Wybierz:', { reply_markup: { inline_keyboard: [['Subskrybuj', 'subskrybuj'], ['Twoje', 'twoje_subskrypcje'], ['Powrót', 'back_to_menu']] } }); }
  ...
  if (sess.mode === 'view' && STREFY.includes(text)) { sess.strefa = text; sess.mode = 'view_filters'; bot.sendMessage(chatId, 'Filtr?', { reply_markup: { inline_keyboard: [['Dzisiaj', 'filter_date_today_' + text], ['Jutro', 'filter_date_tomorrow_' + text], ['Rano', 'filter_time_morning_' + text], ['Popołudnie', 'filter_time_afternoon_' + text], ['Wieczór', 'filter_time_evening_' + text], ['<6h', 'filter_duration_short_' + text], ['Wszystkie', 'filter_all_' + text], ['Powrót', 'back_to_menu']] } }); }
  if (sess.mode === 'oddaj') { if (!sess.strefa && STREFY.includes(text)) { sess.strefa = text; return bot.sendMessage(chatId, 'Data? (np. dzisiaj)', returnKeyboard); } if (sess.strefa && !sess.date) { const date = parseDate(text); if (!date) return sendErr(chatId, sess, 'Zły format daty.'); sess.date = date; return bot.sendMessage(chatId, 'Godziny? (np. 11:00-19:00)', returnKeyboard); } if (sess.date && !sess.time) { const time = parseTime(text); if (!time) return sendErr(chatId, sess, 'Zły format godzin.'); if (await db.get(`SELECT id FROM shifts WHERE username = $1 AND date = $2 AND time = $3 AND strefa = $4`, [username, sess.date, time, sess.strefa])) return sendErr(chatId, sess, 'Duplikat zmiany.'); await db.run(`INSERT INTO shifts (username, chat_id, date, time, strefa, created_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, [username, chatId, sess.date, time, sess.strefa, moment().tz('Europe/Warsaw').format()]); await updateStats(chatId, 'shifts_given'); bot.sendMessage(chatId, `Zapisano: ${sess.date}, ${time}, ${sess.strefa}`, mainKeyboard); await notifySubscribers(sess.strefa, sess.date, time, username, chatId); clearSession(chatId); } }
...
bot.on('callback_query', async (query) => {
  ...
  else if (data.startsWith('edit_')) { sess.shiftId = data.split('_')[1]; sess.mode = 'edit_select'; bot.sendMessage(chatId, 'Co edytować?', { reply_markup: { inline_keyboard: [['Strefa', `edit_strefa_${sess.shiftId}`], ['Data', `edit_date_${sess.shiftId}`], ['Czas', `edit_time_${sess.shiftId}`], ['Powrót', 'back_to_menu']] } }); }
  else if (data.startsWith(['edit_strefa_', 'edit_date_', 'edit_time_'][0])) { sess.mode = 'edit_strefa'; sess.shiftId = data.split('_')[2]; bot.sendMessage(chatId, 'Nowa strefa?', zonesKeyboard); }
  else if (data.startsWith(['edit_strefa_', 'edit_date_', 'edit_time_'][1])) { sess.mode = 'edit_date'; sess.shiftId = data.split('_')[2]; bot.sendMessage(chatId, 'Nowa data? (np. dzisiaj)', returnKeyboard); }
  else if (data.startsWith(['edit_strefa_', 'edit_date_', 'edit_time_'][2])) { sess.mode = 'edit_time'; sess.shiftId = data.split('_')[2]; bot.sendMessage(chatId, 'Nowy czas? (np. 11:00-19:00)', returnKeyboard); }
