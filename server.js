...
// Хелперы
async function cleanup(chatId, showMenu = false) {
  const sess = session[chatId];
  if (sess?.messagesToDelete) for (const id of sess.messagesToDelete) await bot.deleteMessage(chatId, id).catch(() => {});
  if (sess?.userMessages) for (const id of sess.userMessages) await bot.deleteMessage(chatId, id).catch(() => {});
  delete session[chatId];
  if (showMenu) await bot.sendMessage(chatId, 'Cześć! Co chcesz zrobić?', mainKeyboard);
}

async function sendErr(chatId, sess, text) {
  const m = await bot.sendMessage(chatId, text);
  sess.messagesToDelete.push(m.message_id);
}

function parseDate(text) {
  const t = new Date(), jutro = new Date(Date.now() + 864e5);
  if (text.toLowerCase().includes('dzi')) return t.toISOString().split('T')[0];
  if (text.toLowerCase().includes('jutro')) return jutro.toISOString().split('T')[0];
  const match = text.match(/(\d{1,2})[.\-/](\d{1,2})/);
  if (!match) return null;
  const [_, d, m] = match;
  const date = new Date(`${t.getFullYear()}-${m.padStart(2, '0')}-${d.padStart(2, '0')}`);
...
async function notifySubscribers(strefa, date, time) {
  ...
  for (let i = 0; i < rows.length; i++) {
    setTimeout(() => {
      bot.sendMessage(rows[i].user_id, `Nowa zmiana w Twojej strefie ${strefa}:\n${date} ${time}`).catch(() => {});
    }, i * 100);
  }
}